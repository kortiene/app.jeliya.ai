...
on top of that, carries the surviving Phase 1 protocol primitives (D1/D2/D3/D4/D7 —
the D5a control core has been removed); the exact current
...
| Current source candidate | **advanced past** `420698463bff70535d2f450a4f05d573a8ab589f` (public `main`; pre-Phase-1 `922f620…` plus PR #78, remediation PRs #80–#85, conditions PR #89, hardening PR #90, the delta-reviewed issue #91 fix PR #94, the invite-disclosure and workspace-lint PRs #95/#98, the D5b/D6 companion track PRs #100–#106 — **since removed in full** by PR #117 — the postcss bump PR #111, and the Phase-3 governance records) with Iroh Rooms `a5d98b70d717f35d3ce60953a88e12e646f2e871`. The companion-plane removal changes the tree, so no retained signed run binds the current candidate. The exact SHA is recorded at the next signed evidence run; this repository squash-merges, so a branch SHA is not the SHA that lands. **Correction:** the interleaved `main` commits were *not* docs-only — `git diff --stat 4206984 9ab5d43 -- crates ui` reports 50 files changed and 11,978 insertions |
| Last network-qualified `v0.6.0` snapshot | Jeliya `922f620b30ee95c82426a7d4404b1f73a70c0958` (the pre-Phase-1 candidate; signed direct `098c4979` + relay `8bda01e6` bind it — does not transfer to the current candidate (past `4206984`; see the Snapshot boundary)). Prior: `55024a46b3e112796ba2acf1dc408dab26dbba2e` with Iroh Rooms `71fbb5007bef4ce83631c94762ec68c2beef3d79` (tag `v0.1.0-rc.3`) |
| Retained certified evidence | signed schema 2 direct (`098c4979`) and forced-relay (`8bda01e6`) runs binding the pre-Phase-1 candidate `922f620…` + `a5d98b70…`; the earlier `1ca39cfa`/`cf28bc63` runs bound `55024a4…` + `71fbb500…` and are superseded. None transfer to the current candidate (past `4206984`; see the Snapshot boundary) — and the issue #91 fix (per-room `EndpointId`s, room-bound invite discovery) changes exactly the surfaces those runs exercise |
...
| Candidate verification | exact-revision upstream fanout, isolation, and store-degradation tests pass; full Jeliya workspace and 67-assertion loopback suites pass at `922f620…`. At the current candidate (past `4206984`; see the Snapshot boundary) (`cdcae83` Phase 1 primitives plus remediation PRs #80–#85, conditions PR #89, hardening PR #90, and the issue #91 fix PR #94), Phase 1 deliverables D1/D2/D3/D4/D7 plus multi-room reception are implemented and locally tested (D5a was implemented at that revision and has since been removed — see the D5a row below; 124 Rust tests on the post-removal tree, clippy clean under `-D warnings`, UI + docs gates; PR #78 run `29868870066` — a `pull_request` run at branch head `e9f1ed5` — plus merge-SHA push runs `29922951249` at `df28f6a`, `29951799090` at `d610076`, `29957825701` at `dcd940e`, and `29997137062` at `4206984`). Fresh signed direct and forced-relay evidence is still required, and it must bind the **post-removal** candidate — runs against `4206984` cannot qualify a tree from which three crates have since been deleted; the Phase 1 gate is closed — row #7 APPROVE-WITH-CONDITIONS with [risk-owner GO recorded 2026-07-22](phase-1-gate-verdict.md#go-decision--risk-owner-countersignature-2026-07-22), approval [extended to `d610076` and `dcd940e`](phase-1-security-review.md#conditions-delta-review-2026-07-22) and [to `4206984`](phase-1-security-review.md#issue-91-delta-review-2026-07-23) |
...
| Companion control-protocol core (Phase 1 D5a) — **removed** | removed from the tree | historical — the D5a tests were green on PR #78 at `cdcae83`; that evidence describes a revision that no longer exists and certifies nothing in the current candidate | never released | Implemented under Phase 1 D5a (PR #78) as `crates/jeliya-control` — pairing transcript + ~32-bit SAS, non-extractable bounded-lifetime control key (A1), default-deny scopes, sliding replay window, revocation (ADR #2) — and extended by the D5b/D6 companion track (PRs #100–#106). **Removed by the [desktop-operator-first scope decision](desktop-first-scope-decision.md)** (PR #117), which deleted all three companion crates, `crates/jeliyad/src/companion.rs`, the five `--companion-*` flags and `ui/src/lib/control/`. No published release ever contained it, so no user-visible capability was withdrawn. The design and the byte-level wire format are retained, `deprecated`, in the [companion control protocol decision](companion-control-protocol-decision.md) and the [companion control wire protocol](control-wire-protocol.md). |
...
current candidate: fresh signed direct and forced-relay evidence bound to the
post-removal pair (the qualified source commit + `a5d98b70…`), passing
