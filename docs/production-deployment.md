...
- `/.well-known/security.txt` is a static `text/plain` route (RFC 9116),
  ...
  so clients re-fetch it rather than serve a stale copy; keeping the `Expires`
  field itself unexpired is a separate scheduled-refresh task, not a caching
  effect. The `default-src 'none'` CSP does not apply to a directly-fetched text
  file. See the [hosted-origin vulnerability disclosure
...
- DNS, certificate, HTTP redirect, and security headers, including the
  `/.well-known/security.txt` route on `app.jeliya.ai` and `relay-auth.jeliya.ai`
  (RFC 9116, `text/plain`, unexpired `Expires`, expected `Contact`) per the
  [hosted-origin vulnerability disclosure
  record](vulnerability-disclosure-decision.md);
...
- `/.well-known/security.txt` is served in RFC 9116 form on `app.jeliya.ai` and `relay-auth.jeliya.ai`, with an unexpired `Expires` and a `Contact` resolving to the private-reporting URL, and covered by the production smoke tests (#48, [hosted-origin vulnerability disclosure record](vulnerability-disclosure-decision.md));
