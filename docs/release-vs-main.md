...
revision pair; `main` has since advanced through `cdcae83` (Phase 1, PR #78) to
`df28f6a` (Phase 1 remediation), so the release evidence gate is **READY for
`922f620…`** but **requires fresh signed direct/relay runs bound to
`df28f6a`** before any release at the current candidate. The `v0.5.0` manifests
