...
| Current source candidate | Jeliya `922f620b30ee95c82426a7d4404b1f73a70c0958` | exact dependency pin in `Cargo.toml` and `Cargo.lock`; workspace and 67-assertion loopback suites pass; network qualification pending |
...
| Browser-supplied dial hints as an outbound-probe and deanonymization primitive | the companion emits traffic to attacker-chosen socket addresses before any membership check succeeds, disclosing its endpoint and network position. The identity dialled stays ticket-bound — the dial set iterates the ticket's discovery devices and matches caller hints by endpoint id, so a hint cannot substitute a different peer, and a probe address that holds no matching key cannot complete an authenticated session or be persisted as a hint | proposed: an enumerated scope model that either forbids caller-supplied peer hints from a browser controller or requires approval for them | `room.join` accepts a caller-supplied `peers` list of `endpoint-id@ip:port` strings; the endpoint id must match the ticket, but the socket address actually dialed comes from the caller, and successful joins remember the hints. The join-time allowlist binds inbound admission to the inviter identity — it does not constrain the outbound dial addresses, so it is a control against third parties and not against the controller |
...
| Credential minting as an abuse and cost channel | any freely generated keypair converts into paid relay capacity, with the egress cost term unbounded and the cost-ceiling gate unfalsifiable because no numeric ceiling is defined | proposed: per-IP and per-endpoint handshake, connection, byte, and rate limits, plus no arbitrary relay egress and no generic TCP proxying | the architecture defines no minting admission rule. Options were raised in the adversarial review — binding minting to a companion-countersigned control key, a privacy-preserving rate token, or open minting with a hard egress cap and automatic shed — but none is adopted by the decision record and none is a commitment of this architecture. Relay-auth issuance and proof-of-possession behavior is recorded as a highest-risk unknown |
...
- A first-party origin can read everything it renders and act within every
  scope it is granted. The decision record accepts this explicitly as a
  planning assumption. CSP and Trusted Types constrain injected third-party
...
- Amendments **A1**, **A2**, **A3**, and **A6** are open and each blocks the
  phase gate it names. **A4** and **A5** bound browser-peer storage support and
  accessibility and localization scope for the new surfaces; they are recorded
  in the decision record and are not restated as threats here.
