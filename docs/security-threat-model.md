...
| A hostile service worker keeps serving its own cached shell after the CDN pointer is rolled back | rollback restores clean bytes without stopping hostile execution; an installed PWA that is not navigated may not re-check for up to 24 hours | **decided** by [amendment A2 (#44)](hostile-frontend-containment-decision.md): `Clear-Site-Data` as an incident header served on the service-worker script path and `index.html`, plus a signed web-shell kill switch the service worker checks and self-terminates on | the controls are **decided but not yet implemented** — no service worker or header emission exists in the preview tree yet. As A2 records, a hostile worker controls its own fetch handler, so `Clear-Site-Data` delivery is not guaranteed and eviction is backstopped by the browser's out-of-band worker-script update (~24h plus the update-triggering navigation and a subsequent reload); the served header, the SW kill-switch check, and the eviction smoke test are Phase-3 build items that gate the launch |
| The rollback objective measures byte availability, not hostile-code termination | the Phase 3 gate can pass, and a runbook can report containment, while every installed client still executes the attacker's cached shell | **decided** by [amendment A2 (#44)](hostile-frontend-containment-decision.md): the service-objective table states two numbers — 15-minute CDN-pointer rollback (byte availability) and a separate hostile-code-termination objective | the objective is now **split** in the architecture document; the 15-minute number no longer implies hostile code has stopped. A2 is decided; the served controls and the hostile-shell eviction smoke test are Phase-3 build items that gate the launch |
...
| Trusted Types is required without constraining which policies may be created | injected code creates its own conforming policy and satisfies the requirement, so the enforcement reads stronger than it is | **decided** by the [Trusted Types policy allowlist decision](trusted-types-policy-decision.md) (#46): an explicit `trusted-types jeliya-sw` allowlist alongside `require-trusted-types-for 'script'`, excluding `'none'` and a bare `default` policy | the architecture document now specifies `trusted-types jeliya-sw;`. The residual risk is that the directive is not yet **served or exercised** — serving it in the header set and the CSP/Trusted-Types tests are Phase-3 build items. Adoption is cheap because the preview UI uses no `innerHTML`, `dangerouslySetInnerHTML`, `eval`, or `new Function` |
